  ...
  async fetch(ctx: ProviderContext): Promise<QuotaResult> {
    ...
    if (!key) {
      return {
        ...
        needsKey: true,
        message: "No z.ai API key. Paste one below (find it in the z.ai console).",
      };
    }
    ...
    if (res.ok && Array.isArray(limits)) {
      ...
      for (const l of limits) {
        ...
        if (l.type === "TOKENS_LIMIT" && l.unit === 3)
          metrics.push({ label: "Tokens (5h)", used: num(l.percentage), limit: 100, unit: "percent" as const, resetAt });
        if (l.type === "TOKENS_LIMIT" && l.unit === 6)
          metrics.push({ label: "Tokens (month)", used: num(l.percentage), limit: 100, unit: "percent" as const, resetAt });
        if (l.type === "TIME_LIMIT")
          metrics.push({ label: "Web tool (month)", used: num(l.percentage), limit: 100, unit: "percent" as const, resetAt });
      }
      if (metrics.length) {
    ...
    return {
      ...
      needsKey: false,
      message:
        "z.ai key is active, but the coding plan exposes no public quota endpoint. Open the console to see remaining prompts.",
