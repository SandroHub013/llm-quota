...
save(${jsLiteral(caches.antigravity)}, "antigravity", antigravityData);`);
  ...
  // Hoisted out of the summary below: one optional line is not worth a conditional
  // inside a template inside another conditional.
  const zaiLine = caches.zai
    ? `
...
if (Number.isFinite(fiveHour)) parts.push(\`5h \${Math.round(fiveHour)}% used\`);${zaiLine}`;
...
Save-Snapshot ${psLiteral(caches.antigravity)} 'antigravity' $antigravityData`);
  ...
  // Hoisted for the same reason as its JavaScript twin above.
  const zaiLine = caches.zai
    ? `
